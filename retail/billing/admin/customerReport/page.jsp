...
<body>
    <%
        ...
        boolean enquiryMode = "enquiry".equalsIgnoreCase(mode);
    %>
    ...
    <div class="container-fluid mt-4 px-4">
        ...
        <div class="d-flex align-items-center justify-content-between mb-4 no-print">
            <div>
                <h4 class="mb-0 fw-bold"><i class="fas fa-user me-2 text-primary"></i><%= enquiryMode ? "Customer Enquiry Report" : "Customer Analysis Report" %></h4>
                <small class="text-muted"><%= enquiryMode ? "Select customer by name/phone to open bill-wise enquiry details" : "Sales, returns, exchanges &amp; loyalty points by customer" %></small>
            </div>
    ...
    <script>
    const ctx = '<%=ctx%>';
    const enquiryMode = <%= enquiryMode ? "true" : "false" %>;
    let acTimer = null;
    ...
    function selectCustomer(c) {
        ...
        badge.innerHTML = `<span class="badge bg-success clickable-customer" onclick="openCustomerEnquiry()" title="Open customer enquiry">
                                <i class="fas fa-user me-1"></i>${escHtml(c.name)}
                           </span>
                           ${c.phone ? `<span class="badge bg-secondary ms-1 clickable-customer" onclick="openCustomerEnquiry()" title="Open customer enquiry">${escHtml(c.phone)}</span>` : ''}`;

        // In enquiry mode, open enquiry details immediately after selecting customer.
        if (enquiryMode) {
            openCustomerEnquiry();
        }
    }
    ...
    function emptyRow(cols, msg) {
        ...
        function openCustomerEnquiry() {
                ...
                const modal = new bootstrap.Modal(document.getElementById('customerEnquiryModal'));
                document.getElementById('customerEnquiryContent').innerHTML =
                        '<div class="text-center py-5"><div class="spinner-border text-primary" role="status"></div></div>';
                modal.show();

                fetch(ctx + '/admin/customerReport/getCustomerEnquiry.jsp?customerId=' + encodeURIComponent(customerId) +
                            '&fromDate=' + encodeURIComponent(fromDate) + '&toDate=' + encodeURIComponent(toDate))
                .then(r => r.json())
                .then(data => {
                        if (data.error) {
                        ...
                        html += '<thead class="table-light"><tr>' +
                                        ...
                                        '<th class="text-end">Courier</th><th class="text-end">Paid</th><th class="text-end">Due Collection</th>' +
                                        '<th class="text-end">Total Paid</th><th>Download</th><th>Download Date</th><th class="text-end">Photo Count</th>' +
                                        '<th>Delivery Date</th><th>Delivered Date</th><th>Order User</th><th>Delivered User</th><th>Payment Summary</th>' +
                                        '</tr></thead><tbody>';
                        ...
                        rows.forEach((r, i) => {
                                html += '<tr>' +
                                        ...
                                        `<td class="text-end">${fmt(r.initialPaid || 0)}</td>` +
                                        `<td class="text-end">${fmt(r.duePaid || 0)}</td>` +
                                        ...
                                        `<td>${escHtml(r.deliveredUser || '-')}</td>` +
                                        `<td><button class="btn btn-sm btn-outline-primary pay-sum-btn"
                                            data-bill="${escHtml(r.billNo || '-')}">
                                            View
                                            </button></td>` +
                                '</tr>';
                        });
                        ...
                        document.getElementById('customerEnquiryContent').innerHTML = html;
                                ...
                                document.querySelectorAll('.pay-sum-btn').forEach((btn, idx) => {
                                    ...
                                    openPaymentSummary(
                                        btn.getAttribute('data-bill') || '-',
                                        Number(row.initialPaid || 0),
                                        Number(row.duePaid || 0),
                ...
                .catch(() => {
                        ...
                        function openPaymentSummary(billNo, initialPaid, duePaid, totalPaid) {
                document.getElementById('paySummaryBody').innerHTML =
                                `<div class="mb-2"><strong>Bill No:</strong> ${escHtml(billNo || '-')}</div>` +
                        `<div class="mb-1">Paid: <strong>${fmt(initialPaid)}</strong></div>` +
                        `<div class="mb-1">Due Collection: <strong>${fmt(duePaid)}</strong></div>` +
                        `<div class="mb-1">Total Paid: <strong>${fmt(totalPaid)}</strong></div>`;
                new bootstrap.Modal(document.getElementById('paymentSummaryModal')).show();
        }
    </script>

        <div class="modal fade" id="customerEnquiryModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-xl modal-dialog-scrollable">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Customer Enquiry</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body" id="customerEnquiryContent"></div>
