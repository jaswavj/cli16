...
try {
    ...
    if (customerName == null || customerName.trim().isEmpty() || "-".equals(customerName.trim())) {
        response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
        out.print("ERROR: Customer name is required.");
        return;
    }
    if (customerPhn == null || customerPhn.trim().isEmpty() || "-".equals(customerPhn.trim())) {
        response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
        out.print("ERROR: Customer phone number is required.");
        return;
    }
    int customerId = 0;
    ...
    if (isDownloadedStr != null) {
        String normalizedDownloaded = isDownloadedStr.trim().toLowerCase();
        if ("1".equals(normalizedDownloaded)
                || "true".equals(normalizedDownloaded) || "on".equals(normalizedDownloaded)) {
            isDownloaded = 1;
        }
    }
    ...
    String courierFeeStr = request.getParameter("courierFee");
    ...
    if (courierFeeStr != null && !courierFeeStr.trim().isEmpty()) {
        courierFee = Double.parseDouble(courierFeeStr);
    }
    ...
    for (int i = 0; i < products.length(); i++) {
        ...
        double lineCourierFee = p.has("courierFee") ? p.getDouble("courierFee") : 0.0;
