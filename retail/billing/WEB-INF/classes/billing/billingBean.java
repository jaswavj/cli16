...
public String saveBillItems(List<ProductItem> items,
                         ...
                         double payableAmount, double grandTotal,
                         int uid, double priceTotal, double discountTotal,String customerPhn
                         	,double totalPaid,double cashPaid,double bankPaid,int mode,int type,double balance,int customerId,double courierFee) throws Exception {
    return saveBillItems(items, customerName, finalDiscount, payableAmount, grandTotal, uid, priceTotal, discountTotal, customerPhn, totalPaid, cashPaid, bankPaid, mode, type, balance, customerId, 3, 0, 1, courierFee); // Default to Retailer, no attender, tax bill ON
}
...
public String saveBillItems(List<ProductItem> items,
                         ...
                         double payableAmount, double grandTotal,
                         int uid, double priceTotal, double discountTotal,String customerPhn
                         	,double totalPaid,double cashPaid,double bankPaid,int mode,int type,double balance,int customerId,int priceCategory,double courierFee) throws Exception {
    return saveBillItems(items, customerName, finalDiscount, payableAmount, grandTotal, uid, priceTotal, discountTotal, customerPhn, totalPaid, cashPaid, bankPaid, mode, type, balance, customerId, priceCategory, 0, 1, courierFee); // No attender, tax bill ON
}
...
public String saveBillItems(List<ProductItem> items,
                         ...
                         double payableAmount, double grandTotal,
                         int uid, double priceTotal, double discountTotal,String customerPhn
                         	,double totalPaid,double cashPaid,double bankPaid,int mode,int type,double balance,int customerId,int priceCategory,int attenderId,double courierFee) throws Exception {
    return saveBillItems(items, customerName, finalDiscount, payableAmount, grandTotal, uid, priceTotal, discountTotal, customerPhn, totalPaid, cashPaid, bankPaid, mode, type, balance, customerId, priceCategory, attenderId, 1, courierFee); // Tax bill ON
}
...
public String saveBillItems(List<ProductItem> items,
    ...
    return saveBillItems(items, customerName, finalDiscount, payableAmount, grandTotal, uid, priceTotal, discountTotal, customerPhn, totalPaid, cashPaid, bankPaid, mode, type, balance, customerId, priceCategory, attenderId, isTaxBill, 0.0);
}
...
ps.executeUpdate();
        ...
        String sqlDetail = "INSERT INTO prod_bill_details (bill_id, prod_id, qty, price, disc, total, gst, cost, commission, courier_fee) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        for (ProductItem item : items) {
            ...
            ps.setDouble(10, item.courierFee);
            ps.addBatch();
        }
...
public String updatePendingDelivery(int billId, String deliveryDate, int isDownloaded, int photoCount) throws Exception {
    ...
    try {
        ...
        String verifySql = "SELECT delivery_date, IFNULL(is_downloaded,0) AS is_downloaded, download_date FROM prod_bill WHERE id = ?";
        verifyPs = con.prepareStatement(verifySql);
        verifyPs.setInt(1, billId);
        verifyRs = verifyPs.executeQuery();

        boolean verified = false;
        if (verifyRs.next()) {
            java.sql.Date actualDeliveryDate = verifyRs.getDate("delivery_date");
            int actualDownloaded = verifyRs.getInt("is_downloaded");
            java.sql.Date actualDownloadDate = verifyRs.getDate("download_date");

            boolean deliveryOk;
            if (deliveryDate != null && !deliveryDate.trim().isEmpty()) {
                deliveryOk = actualDeliveryDate != null && deliveryDate.equals(actualDeliveryDate.toString());
            } else {
                deliveryOk = actualDeliveryDate == null;
            }

            boolean downloadedOk = actualDownloaded == (isDownloaded == 1 ? 1 : 0);
            boolean downloadDateOk = (isDownloaded == 1) ? (actualDownloadDate != null) : (actualDownloadDate == null);
            verified = deliveryOk && downloadedOk && downloadDateOk;
        }

        if (!verified) {
            con.rollback();
            return "Update verification failed. Rolled back.";
        }
        ...
        if (verifyRs != null) {
            try { verifyRs.close(); } catch (SQLException e) { ; }
            verifyRs = null;
        }
        if (verifyPs != null) {
            try { verifyPs.close(); } catch (SQLException e) { ; }
            verifyPs = null;
        }
        if (ps != null) {
...
public String markBillDelivered(int billId, String deliveryPlace, String deliveredDate, String deliveryPerson) throws Exception {
    Connection con = null;
    PreparedStatement ps = null;
    PreparedStatement verifyPs = null;
    ResultSet verifyRs = null;
    ...
    try {
        ...
        String sql = "UPDATE prod_bill SET delivery_place = ?, delivered_date = ?, delivery_person = ?, is_delivered = 1 " +
                     "WHERE id = ? AND IFNULL(is_cancelled,0)=0 AND IFNULL(is_delivered,0)=0";
        ps = con.prepareStatement(sql);
        ps.setString(1, deliveryPlace != null ? deliveryPlace.trim() : "");
        if (deliveredDate != null && !deliveredDate.trim().isEmpty()) {
            ps.setDate(2, java.sql.Date.valueOf(deliveredDate.trim()));
        } else {
            ps.setNull(2, java.sql.Types.DATE);
        }
        ps.setString(3, deliveryPerson != null ? deliveryPerson.trim() : "");
        ps.setInt(4, billId);
        ...
        String verifySql = "SELECT IFNULL(delivery_place,'') AS delivery_place, delivered_date, IFNULL(delivery_person,'') AS delivery_person, IFNULL(is_delivered,0) AS is_delivered FROM prod_bill WHERE id = ?";
        verifyPs = con.prepareStatement(verifySql);
        verifyPs.setInt(1, billId);
        verifyRs = verifyPs.executeQuery();
        ...
        boolean verified = false;
        if (verifyRs.next()) {
            String actualPlace = verifyRs.getString("delivery_place");
            java.sql.Date actualDate = verifyRs.getDate("delivered_date");
            String actualPerson = verifyRs.getString("delivery_person");
            int actualDelivered = verifyRs.getInt("is_delivered");

            boolean placeOk = (actualPlace != null ? actualPlace : "").equals(deliveryPlace != null ? deliveryPlace.trim() : "");
            boolean dateOk;
            if (deliveredDate != null && !deliveredDate.trim().isEmpty()) {
                dateOk = actualDate != null && deliveredDate.trim().equals(actualDate.toString());
            } else {
                dateOk = actualDate == null;
            }
            boolean personOk = (actualPerson != null ? actualPerson : "").equals(deliveryPerson != null ? deliveryPerson.trim() : "");
            boolean deliveredOk = actualDelivered == 1;
            verified = placeOk && dateOk && personOk && deliveredOk;
        }

        if (!verified) {
            con.rollback();
            return "Delivery update verification failed. Rolled back.";
        }
...
public Vector getBillDetailsUsingNo(String bill) throws Exception {
    ...
    try {
        ...
        String sql = "SELECT c.`name`,b.`qty`,b.`price`,b.`disc`,b.`total`,b.gst,d.name AS category_name,c.hsn,IFNULL(u.name,'') AS unit_name,IFNULL(u.convertion_unit,'') AS convertion_unit,IFNULL(b.courier_fee,0) AS courier_fee FROM `prod_bill` a "
        ...
        while (rs.next()) {
            ...
            row.addElement(rs.getString(11)); // courier_fee
...
public Vector getBillAmount(int id) throws Exception {
    ...
    try {
        ...
        String sql =
            "SELECT a.payable, a.paid, a.currentBalance, a.bill_display " +
...
public Vector getDuePaidList(int id) throws Exception {
    ...
    try {
        ...
        String sql = "SELECT b.`cusName`,a.`balance`,a.`paid`,a.`finalBalance`,CASE WHEN a.mode=1 THEN 'Cash' ELSE 'Bank' END AS MODE, "
					...
					+"	WHEN a.bankOption=4 THEN 'NEFT' WHEN a.bankOption=5 THEN 'WALLET' END AS bank,DATE_FORMAT(a.date,'%d-%m-%Y'),a.time,c.user_name   "
...
public String getProductHistory(int productId, int customerId) throws Exception {
        ...
        try {
            ...
            String sql = "SELECT pb.bill_display, pb.date, pb.time, pbd.qty, pbd.price, pbd.disc, COALESCE(pbd.courier_fee, 0) AS courier_fee, pbd.total, pb.cusName " +
                         "FROM prod_bill pb " +
            ...
            sql += "ORDER BY pb.date DESC, pb.time DESC LIMIT 10";
            ...
            while (rs.next()) {
                ...
                json.append("\"courierFee\":").append(rs.getDouble("courier_fee")).append(",");
...
public JSONObject saveFollowNote(String customerName, String customerPhn, String customerIdStr, String description, int uid) throws Exception {
    ...
    try {
        ...
        if (customerPhn == null || customerPhn.trim().isEmpty()) {
            result.put("success", false);
        ...
        if (description == null || description.trim().isEmpty()) {
            result.put("success", false);
            result.put("message", "Description is required.");
            return result;
        }

        customerName = customerName.trim();
        customerPhn = customerPhn.trim();
        ...
        if (customerId == 0) {
            ...
            int existingId = prod.checkTheCustomerNameExist(customerName);
            if (existingId != 0) {
        ...
        if (customerId == 0) {
            result.put("success", false);
            result.put("message", "Could not determine customer ID. Please try again.");
            return result;
        }

        con = util.DBConnectionManager.getConnectionFromPool();
        ps = con.prepareStatement("SELECT id FROM notes WHERE customer_id = ? AND is_closed = 0 LIMIT 1");
        ps.setInt(1, customerId);
        rs = ps.executeQuery();
        if (rs.next()) {
            result.put("success", false);
            result.put("message", "This customer already has an open follow note. Please close it first.");
            return result;
        }
        ...
        con.setAutoCommit(false);
        ps = con.prepareStatement("INSERT INTO notes (customer_id, description, uid, dateTime, is_closed) VALUES (?, ?, ?, NOW(), 0)");
        ps.setInt(1, customerId);
        ...
        if (rows > 0) {
            ...
            result.put("message", "Follow note saved successfully.");
        } else {
            con.rollback();
            result.put("success", false);
            result.put("message", "Failed to insert note.");
        }
...
public JSONArray getFollowNotes() throws Exception {
    ...
    try {
        con = util.DBConnectionManager.getConnectionFromPool();
        ps = con.prepareStatement(
            "SELECT n.id, c.name AS customerName, c.phone_number AS customerPhone, " +
            "n.description, n.dateTime, n.is_closed, n.closed_dateTime, " +
            "u.fullName AS addedBy, cu.fullName AS closedBy " +
            "FROM notes n " +
            "JOIN customers c ON n.customer_id = c.id " +
            "LEFT JOIN users u ON n.uid = u.id " +
            "LEFT JOIN users cu ON n.closed_uid = cu.id " +
            "ORDER BY n.is_closed ASC, n.id DESC"
        );
        rs = ps.executeQuery();
        ...
        while (rs.next()) {
            ...
            obj.put("customerName", rs.getString("customerName") != null ? rs.getString("customerName") : "");
            obj.put("customerPhone", rs.getString("customerPhone") != null ? rs.getString("customerPhone") : "");
            obj.put("description", rs.getString("description") != null ? rs.getString("description") : "");
            obj.put("dateTime", rs.getString("dateTime") != null ? rs.getString("dateTime") : "");
            obj.put("isClosed", rs.getInt("is_closed"));
            obj.put("addedBy", rs.getString("addedBy") != null ? rs.getString("addedBy") : "");

            String closedAt = "";
            if (rs.getInt("is_closed") == 1) {
                String closedDT = rs.getString("closed_dateTime");
                String closedBy = rs.getString("closedBy");
                if (closedDT != null) closedAt += closedDT;
                if (closedBy != null) closedAt += (closedAt.isEmpty() ? "" : " by ") + closedBy;
            }
...
public JSONObject checkOpenFollowNote(int customerId) throws Exception {
    JSONObject result = new JSONObject();
    Connection con = null;
    PreparedStatement ps = null;
    ResultSet rs = null;
    ...
    try {
        con = util.DBConnectionManager.getConnectionFromPool();
        ps = con.prepareStatement(
            "SELECT n.description, u.fullName AS userName " +
            "FROM notes n " +
            "LEFT JOIN users u ON n.uid = u.id " +
            "WHERE n.customer_id = ? AND n.is_closed = 0 LIMIT 1"
        );
        ps.setInt(1, customerId);
        rs = ps.executeQuery();

        if (rs.next()) {
            result.put("hasOpen", true);
            result.put("description", rs.getString("description") != null ? rs.getString("description") : "");
            result.put("userName", rs.getString("userName") != null ? rs.getString("userName") : "A user");
        } else {
            result.put("hasOpen", false);
        }
...
public JSONObject closeFollowNote(int noteId, int closedUid) throws Exception {
    JSONObject result = new JSONObject();
    Connection con = null;
    PreparedStatement ps = null;
    ...
    try {
        ...
        con.setAutoCommit(false);
        ps = con.prepareStatement(
            "UPDATE notes SET is_closed = 1, closed_dateTime = NOW(), closed_uid = ? " +
            "WHERE id = ? AND is_closed = 0"
        );
        ps.setInt(1, closedUid);
        ps.setInt(2, noteId);
        ...
        if (rows > 0) {
            con.commit();
            result.put("success", true);
            result.put("message", "Note closed successfully.");
        } else {
            con.rollback();
            result.put("success", false);
            result.put("message", "Note not found or already closed.");
        }
...
// Returns [date, cash, bank, paymentTypeName, balance] for the initial bill payment
...
public JSONArray getCustomerEnquiry(int customerId, String fromDate, String toDate) throws Exception {
    ...
    try {
        ...
        billSql.append("SELECT b.id, b.bill_display, b.date, b.time, IFNULL(b.paid,0) AS paid, ");
        billSql.append("IFNULL(b.is_downloaded,0) AS is_downloaded, b.download_date, IFNULL(b.photo_count,0) AS photo_count, ");
        billSql.append("b.delivery_date, b.delivered_date, IFNULL(u.user_name,'-') AS order_user, IFNULL(b.delivery_person,'-') AS delivered_user ");
        billSql.append("FROM prod_bill b ");
        billSql.append("LEFT JOIN users u ON u.id = b.uid ");
        billSql.append("WHERE IFNULL(b.is_cancelled,0)=0 AND IFNULL(b.customerId,0)=? ");

        boolean hasFrom = fromDate != null && !fromDate.trim().isEmpty();
        boolean hasTo = toDate != null && !toDate.trim().isEmpty();
        if (hasFrom && hasTo) {
            billSql.append("AND b.date BETWEEN ? AND ? ");
        }
        billSql.append("ORDER BY b.date DESC, b.time DESC");

        psBills = con.prepareStatement(billSql.toString());
        psBills.setInt(1, customerId);
        if (hasFrom && hasTo) {
            psBills.setString(2, fromDate.trim());
            psBills.setString(3, toDate.trim());
        }

        String itemSql = "SELECT IFNULL(GROUP_CONCAT(p.name SEPARATOR ', '), '-') AS item_names, " +
                         "IFNULL(SUM(bd.price),0) AS price_total, IFNULL(SUM(IFNULL(bd.courier_fee,0)),0) AS courier_total " +
                         "FROM prod_bill_details bd " +
                         "JOIN prod_product p ON p.id = bd.prod_id " +
                         "WHERE bd.bill_id = ?";
        psItems = con.prepareStatement(itemSql);

        String initSql = "SELECT IFNULL(SUM(IFNULL(cash,0) + IFNULL(bank,0)),0) AS initial_paid FROM prod_bill_payment WHERE bill_id = ?";
        psInit = con.prepareStatement(initSql);

        String dueSql = "SELECT IFNULL(SUM(paid),0) AS due_paid FROM prod_bill_due_collection WHERE bill_id = ?";
        psDue = con.prepareStatement(dueSql);

        rsBills = psBills.executeQuery();
        while (rsBills.next()) {
            int billId = rsBills.getInt("id");

            String itemNames = "-";
            double priceTotal = 0.0;
            double courierTotal = 0.0;

            psItems.setInt(1, billId);
            rsItems = psItems.executeQuery();
            if (rsItems.next()) {
                itemNames = rsItems.getString("item_names") != null ? rsItems.getString("item_names") : "-";
                priceTotal = rsItems.getDouble("price_total");
                courierTotal = rsItems.getDouble("courier_total");
            }
            if (rsItems != null) {
                try { rsItems.close(); } catch (Exception ignore) {}
                rsItems = null;
            }

            double initialPaid = 0.0;
            psInit.setInt(1, billId);
            rsInit = psInit.executeQuery();
            if (rsInit.next()) {
                initialPaid = rsInit.getDouble("initial_paid");
            }
            if (rsInit != null) {
                try { rsInit.close(); } catch (Exception ignore) {}
                rsInit = null;
            }

            double duePaid = 0.0;
            psDue.setInt(1, billId);
            rsDue = psDue.executeQuery();
            if (rsDue.next()) {
                duePaid = rsDue.getDouble("due_paid");
            }
            if (rsDue != null) {
                try { rsDue.close(); } catch (Exception ignore) {}
                rsDue = null;
            }
            double totalPaid = initialPaid + duePaid;

            java.sql.Date dlDate = rsBills.getDate("download_date");
            java.sql.Date deliveryDate = rsBills.getDate("delivery_date");
            java.sql.Date deliveredDate = rsBills.getDate("delivered_date");

            JSONObject row = new JSONObject();
            row.put("billNo", rsBills.getString("bill_display"));
            row.put("date", rsBills.getString("date"));
            row.put("time", rsBills.getString("time"));
            row.put("itemNames", itemNames);
            row.put("priceTotal", priceTotal);
            row.put("courierTotal", courierTotal);
            row.put("initialPaid", initialPaid);
            row.put("duePaid", duePaid);
            row.put("totalPaid", totalPaid);
            row.put("isDownloaded", rsBills.getInt("is_downloaded"));
            row.put("downloadDate", dlDate != null ? dlDate.toString() : "");
            row.put("photoCount", rsBills.getInt("photo_count"));
            row.put("deliveryDate", deliveryDate != null ? deliveryDate.toString() : "");
            row.put("deliveredDate", deliveredDate != null ? deliveredDate.toString() : "");
            row.put("orderUser", rsBills.getString("order_user"));
            row.put("deliveredUser", rsBills.getString("delivered_user"));
            rows.put(row);
        }

        return rows;
    } finally {
        if (rsInit != null) try { rsInit.close(); } catch (Exception e) {}
        if (rsDue != null) try { rsDue.close(); } catch (Exception e) {}
        if (rsItems != null) try { rsItems.close(); } catch (Exception e) {}
        if (rsBills != null) try { rsBills.close(); } catch (Exception e) {}
        if (psInit != null) try { psInit.close(); } catch (Exception e) {}
        if (psDue != null) try { psDue.close(); } catch (Exception e) {}
        if (psItems != null) try { psItems.close(); } catch (Exception e) {}
        if (psBills != null) try { psBills.close(); } catch (Exception e) {}
        if (con != null) try { con.close(); } catch (Exception e) {}
    }
}

public JSONObject getBillPaymentSummary(String billNo) throws Exception {
    JSONObject out = new JSONObject();
    JSONArray rows = new JSONArray();
    double totalPaid = 0.0;

    Vector initialPayment = getInitialBillPayment(billNo);
    int billId = getBillId(billNo);
    Vector dueCollections = getDuePaidList(billId);

    boolean hasInitialPay = initialPayment != null && initialPayment.size() >= 5;
    double ipCash = 0.0;
    double ipBank = 0.0;
    double ipBalanceVal = 0.0;
    String ipDate = "-";
    String ipPayType = "-";

    if (hasInitialPay) {
        ipDate = initialPayment.get(0) != null ? initialPayment.get(0).toString() : "-";
        ipPayType = initialPayment.get(3) != null ? initialPayment.get(3).toString() : "-";

        try { ipCash = initialPayment.get(1) != null ? Double.parseDouble(initialPayment.get(1).toString()) : 0.0; } catch (Exception e) { ipCash = 0.0; }
        try { ipBank = initialPayment.get(2) != null ? Double.parseDouble(initialPayment.get(2).toString()) : 0.0; } catch (Exception e) { ipBank = 0.0; }
        try { ipBalanceVal = initialPayment.get(4) != null ? Double.parseDouble(initialPayment.get(4).toString()) : 0.0; } catch (Exception e) { ipBalanceVal = 0.0; }
    ...
    if (dueCollections != null) {
        for (int i = 0; i < dueCollections.size(); i++) {
            Vector dc = (Vector) dueCollections.get(i);
            String dcDate = dc.size() > 6 && dc.get(6) != null ? dc.get(6).toString() : "-";
            String dcMode = dc.size() > 4 && dc.get(4) != null ? dc.get(4).toString() : "-";
            String dcBank = dc.size() > 5 && dc.get(5) != null ? dc.get(5).toString() : "-";

            double dcPaid = 0.0;
            double dcBalance = 0.0;
            try { dcPaid = dc.size() > 2 && dc.get(2) != null ? Double.parseDouble(dc.get(2).toString()) : 0.0; } catch (Exception e) { dcPaid = 0.0; }
            try { dcBalance = dc.size() > 3 && dc.get(3) != null ? Double.parseDouble(dc.get(3).toString()) : 0.0; } catch (Exception e) { dcBalance = 0.0; }

            JSONObject row = new JSONObject();
            row.put("date", dcDate);
            row.put("mode", dcMode);
            row.put("method", dcBank);
            row.put("paid", dcPaid);
            row.put("balance", dcBalance);
            rows.put(row);
            totalPaid += dcPaid;
        }
    }

    out.put("billNo", billNo);
    out.put("rows", rows);
